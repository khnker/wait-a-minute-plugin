"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.sessionExecutions = exports.default = void 0;var _engine = await jitiImport("./engine.js");
var _executionEngine = await jitiImport("./execution-engine.js");
var _cognitionStore = await jitiImport("./cognition-store.js");
var _messageHandler = await jitiImport("./runtime/message-handler.js");

var _memory = await jitiImport("./memory.js");
var _context = await jitiImport("./context.js");
var _assembly = await jitiImport("./assembly.js");
var _verification = await jitiImport("./verification.js");
var _contextDecisionAudit = await jitiImport("./context-decision-audit.js");
var _runtimeGuards = await jitiImport("./runtime-guards.js");
var _riskEngine = await jitiImport("./risk-engine.js");
var _executionState = await jitiImport("./execution-state.js");
var _contextSnapshot = await jitiImport("./context-snapshot.js");
var _nodeFs = _interopRequireDefault(await jitiImport("node:fs"));
var _nodePath = _interopRequireDefault(await jitiImport("node:path"));function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}

const sessionExecutions = exports.sessionExecutions = new Map();
...
async function bridgeExecution(input) {
  ...
  const sessionID = input?.sessionID || input?.sessionId || (0, _context.getSessionId)();
  ...
  try {
    const result = await (0, _executionEngine.startExperiment)(taskRoot, taskId, {
...
const ACTIVE_FILE = (root) => _nodePath.default.join(root || process.cwd(), ".wam", "active-task");

function readActiveTaskId(root) {
  try {
    const v = _nodeFs.default.readFileSync(ACTIVE_FILE(root), "utf-8").trim();
    if (!v) return null;
    try {
...
function readActiveTaskRecord(root) {
  try {
    const raw = _nodeFs.default.readFileSync(ACTIVE_FILE(root), "utf-8").trim();
    if (!raw) return null;
    try {
...
function writeActiveTaskId(id, root) {
  _nodeFs.default.mkdirSync(_nodePath.default.dirname(ACTIVE_FILE(root)), { recursive: true });
  _nodeFs.default.writeFileSync(ACTIVE_FILE(root), JSON.stringify({ id, ts: Date.now() }));
}

function listTaskIds(root) {
  const dir = _nodePath.default.join(root || process.cwd(), ".wam", "tasks");
  try {
    return _nodeFs.default.
...
function taskDir(root, taskId) {
  return _nodePath.default.join(root || process.cwd(), ".wam", "tasks", taskId);
}

function liveFileFor(root, taskId) {
  return _nodePath.default.join(taskDir(root, taskId), "context.md");
}

function readLiveContext(root, taskId) {
  const perSession = liveFileFor(root, taskId);
  try {
    if (_nodeFs.default.existsSync(perSession)) return _nodeFs.default.readFileSync(perSession, "utf-8").trim();
  } catch {}
  return "";
}
...
function persistLiveContext(taskId, state, root) {
  (0, _memory.updateLiveContext)(taskId, state, root);
}
...
function projectState(analysis) {
  return {
    contract: {
      ...
      (0, _engine.buildAssumptions)(analysis.assumed || [])
    },
    questions: [],
...
function writeCavemanSummary(taskId, state, root, extra = "") {
  ...
  const cav = (0, _engine.cavemanify)([...base, extra].filter(Boolean).join("\n"));
  try {
    const dir = _nodePath.default.join(root || process.cwd(), ".wam", "tasks", taskId);
    _nodeFs.default.mkdirSync(dir, { recursive: true });
    _nodeFs.default.writeFileSync(_nodePath.default.join(dir, "caveman-summary.md"), cav + "\n");
  } catch {}
  return cav;
}
...
function loadReferenceEvidence(taskRoot, intent) {
  const refsFile = _nodePath.default.join(taskRoot || "", ".wam", "references.json");
  if (!_nodeFs.default.existsSync(refsFile)) return [];
  try {
    const refs = JSON.parse(_nodeFs.default.readFileSync(refsFile, "utf-8"));
...
const WaitAMinutePlugin = async (pluginInput) => {
  ...
  const wamRootFor = async (sessionID, promptText) => {
    ...
    return (0, _context.resolveWamRoot)(promptText, base);
  };
  ...
  return {
    ...
    (0, _messageHandler.handleMessage)(input, output, {
      bypassed,
      sessionTasks,
      sessionStore,
      cfg,
      ...
      writeActiveTaskId,
      waitAMinute,
      migrateLegacyCognition: _cognitionStore.migrateLegacyCognition,
      noteSuccess: _executionEngine.noteSuccess,
      noteFailure: _executionEngine.noteFailure,
      getTaskState: _engine.getTaskState,
      persistTaskState: _engine.persistTaskState,
      findDuplicateTask: _engine.findDuplicateTask,
      escalateAssumptions: _engine.escalateAssumptions,
      buildAssumptions: _engine.buildAssumptions,
      initMemory: _memory.initMemory,
      updateProjectMemo: _memory.updateProjectMemo,
      updateLiveContext: _memory.updateLiveContext,
      readLiveContext,
      persistLiveContext,
      assembleContext: _assembly.assembleContext,
      createSnapshot: _contextSnapshot.createSnapshot,
      checkContinuation: _contextSnapshot.checkContinuation,
      rebuildScope: _contextSnapshot.rebuildScope,
      getDecision: _memory.getDecision,
      recordDecision: _memory.recordDecision,
      compactDecisions: _memory.compactDecisions,
      writeCavemanSummary,
      truncate,
      delegationLines,
      updateTaskMemory: _memory.updateTaskMemory,
      addRecentChange: _memory.addRecentChange,
      closeSession: _context.closeSession,
      getSessionId: _context.getSessionId,
    ...
    "tool.execute.before": async (input, output) => {
      try {
        ...
        let st = (0, _engine.getTaskState)(taskId, taskRoot);
        ...
        if (!st && !sid) {
          ...
          if (activeId && activeId !== taskId) {
            taskId = activeId;
            st = (0, _engine.getTaskState)(taskId, taskRoot);
          }
        }
        ...
        if (st?.approvedStrategy && st.approvedStrategy.status === "ACTIVE") {
          ...
          if (stratCheck.covered === true) {
            ...
            throw new _riskEngine.WamPolicyBlock(directive, {
              tool,
        ...
        if (risk.level === "BLOCKED") {
          ...
          input.output = directive;
          throw new _riskEngine.WamPolicyBlock(directive, { tool, reason: risk.reason, level: risk.level });
        }
        ...
        if (MUTATING_TOOLS.has(tool) && st?.contract?.status !== "APPROVED" && st?.phase !== "DONE") {
          ...
          input.output = directive;
          throw new _riskEngine.WamPolicyBlock(directive, { tool, reason: "contract not approved", level: "BLOCKED", source: "governance-enforcement" });
        }
...
async function postToolExecution(input, output) {
  try {
    ...
    if (input?.tool && output?.error) {
      try {
        await (0, _executionEngine.noteFailure)(wamRoot, taskId, {
      ...
      try {
        await (0, _executionEngine.noteSuccess)(wamRoot, taskId, {
          hypothesisId: input._wamHypothesisId || mapping.hypothesisId || input.hypothesisId,
          experimentId: input._wamExperimentId || mapping.experimentId || input.experimentId,
          requirementId: input._wamRequirementId || mapping.requirementId || input.requirementId,
...
function wamCli(args, cfg = {}, root = process.cwd(), taskId = readActiveTaskId(root) || "default-task") {
  ...
  if (sub === "skills") {
    ...
    if (action === "explain") {
      ...
      const sel = (0, _engine.routeSkillsV2)(prompt, {}, waitAMinute.getRegistry(), "STANDARD");
  ...
  if (sub === "strategy") {
    if (action === "set") {
      ...
      const st = (0, _engine.getTaskState)(taskId, root);
      ...
      (0, _engine.persistTaskState)(taskId, st, root);
  ...
  if (sub === "status") {
    ...
    return (0, _executionState.getStatusReport)(st, { experiments, failures, pendingAuth, verification, completionEvidence });
  }

  if (sub === "backlog") {
    const st = (0, _engine.getTaskState)(taskId, root);
    if (!st) return "Sin estado de tarea";
    const formatted = (0, _engine.formatBacklog)(st);
  ...
  if (sub === "task") {
    ...
    if (action === "switch") {
      ...
      const st = (0, _engine.getTaskState)(id);
      return st ?
  ...
  if (sub === "ctx") {
    if (action === "list") {
      const caps = (0, _context.listCapsules)(root);
    ...
    if (action === "get") {
      ...
      const r = (0, _context.retrieveContext)(q, { root });
    ...
    if (action === "show") {
      const id = rest.join(" ");
      const c = (0, _context.getCapsule)(id, root);
    ...
    if (action === "promote") {
      ...
      const r = (0, _context.promoteCapsule)(id, target, { approvedBy: approved === "approved" ? "user" : "", root });
      if (!r.ok) return `Promoción rechazada: ${r.reason}`;
      return `Promovida ${id} → ${target} [${r.capsule.provenance}]`;
    }
    if (action === "session") {
      const sid = (0, _context.getSessionId)(root);
      const caps = (0, _context.listCapsules)(process.cwd(), { sessionId: sid });
      return `session: ${sid}\ncapsules de esta sesión: ${caps.length}\nL1 base: ${(0, _context.listCapsules)(process.cwd(), { level: "L1", lifecycle: "active" }).length} cápsula(s)`;
    }
    if (action === "migrate") {
      const dryRun = rest.includes("--dry-run");
      const r = (0, _context.migrateLegacyCapsules)(root, { dryRun });
    ...
    if (sub === "decision") {
      ...
      const tracer = new _contextDecisionAudit.ContextDecisionTracer(`decision-${Date.now()}`, root);
...
const waitAMinute = {
  ...
  analyze: async function ({
    ...
    const result = await (0, _engine.analyze)({
  ...
  loadBundledRegistry: function () {
    try {
      const registryPath = _nodePath.default.join("/home/nicolas/dev/wait-a-minute-plugin", "skills", "registry.json");
      if (!_nodeFs.default.existsSync(registryPath)) return {};
      const entries = JSON.parse(_nodeFs.default.readFileSync(registryPath, "utf-8"));
      const reg = {};
  ...
  buildPersistedState: function (taskId, analysis, root) {
    const existing = (0, _engine.getTaskState)(taskId, root);
    if (existing && existing.requirements && existing.requirements.length) {
      ...
      (0, _engine.persistTaskState)(taskId, merged, root);
      return merged;
    }
    const fresh = {
    ...
    (0, _engine.persistTaskState)(taskId, fresh, root);
    return fresh;
  },
  ...
  evaluateCompletionGate: function (state, prompt) {
    ...
    if (checks.length > 0) {
      ...
      for (const [reqId, list] of byReq) {
        const verdict = (0, _verification.evaluateRequirement)(list, list);
  ...
  resumeTask: function (taskId, root) {
    const state = (0, _engine.getTaskState)(taskId, root);
  ...
  approveContract: function (taskId, root) {
    ...
    (0, _engine.persistTaskState)(taskId, state, root);
    try {
      (0, _memory.updateLiveContext)(taskId, state, root);
    } catch {}
  ...
  markRequirement: async function (taskId, reqId, status, evidence, root) {
    ...
    if (status === "verified" && checksForReq.length > 0) {
      const verdictResult = await (0, _verification.verifyRequirement)(checksForReq, { timeout_ms: 60000 });
  ...
  loadSkillOnDemand: function (skillId, registry, baseDir) {
    return (0, _engine.loadSkillOnDemand)(skillId, registry, baseDir);
  },
...
WaitAMinutePlugin; /* v9-189a108e7d79e894 */
